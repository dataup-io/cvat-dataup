...
import {
    ...
    SerializedQualityReportData, APIQualityReportsFilter, APIAnalyticsEventsFilter,
    APIAnalyticsJSONFilter, APIConsensusSettingsFilter,
...
Axios.interceptors.request.use((reqConfig) => {
    ...
    // For DataUp API requests, use organization_uuid instead of org
    if (reqConfig.url.includes('/dataup')) {
        const { organizationUuid } = config.organization;
        if (organizationUuid) {
            reqConfig.params = { organization_uuid: organizationUuid, ...(reqConfig.params || {}) };
        }
        return reqConfig;
    }
...
function setAuthData(response: AxiosResponse): void {
    ...
    // Store authentication token if present in response data
    if (response.data && response.data.key) {
        store.set('auth_token', response.data.key);
    }
}
...
async function getAnalyticsJSON(params: APIAnalyticsJSONFilter): Promise<any> {
    const { backendAPI } = config;
    const organizationParams = enableOrganization();
    ...
    const response = await Axios.get(`${backendAPI}/events/analytics`, {
        params: finalParams,
    });
...
async function getClassDistribution(filter: APIClassDistributionFilter): Promise<SerializedClassDistribution> {
    const { backendAPI } = config;

    try {
        const response = await Axios.get(`${backendAPI}/analytics/class_distribution`, {
            params: {
                ...filter,
            },
        });
