...
export interface APIAnalyticsJSONFilter extends APIAnalyticsEventsFilter {
    format?: 'raw' | 'aggregated' | 'summary';
    group_by?: 'scope' | 'user_id' | 'project_id' | 'task_id' | 'job_id';
}

export type AnalyticsEventsFilter = CamelizedV2<APIAnalyticsEventsFilter>;
export type AnalyticsJSONFilter = CamelizedV2<APIAnalyticsJSONFilter>;


// export type AnalyticsReportFilter = Camelized<APIAnalyticsReportFilter>;

export interface SerializedClassDistributionEntry {
    class: string;
    count: number;
}
...
export interface APIClassDistributionFilter {
    project_id?: number;
    task_id?: number;
    job_id?: number;
}

export type ClassDistributionFilter = Camelized<APIClassDistributionFilter>;

export interface SerializedClassDistribution {
    class_distribution?: SerializedClassDistributionEntry[];
}
...
export interface SerializedAgentAPI {
    id: string;
    name: string;
    endpoint: string;
    auth_token: string;
    ...
    labels: string[];
    label_source: string;
    owner_id: string;                    // ✅ Changed from owner object to owner_id
    organization_id: string;             // ✅ Changed from organization to organization_id
    is_active: boolean;
    created_date: string;
    updated_date: string;
    last_used: string | null;
    total_usage_count: number;           // ✅ Changed from usage_count
    error_count: number;
    last_error: string | null;
    resolved_labels: string[];           // ✅ New field
    // Remove these old fields:
    // owner: SerializedUser | null;
    // organization: number | null;
    // organization_usage: object | null;
    // usage_count: number;
}

export interface APIAgentAPIsFilter extends APICommonFilterParams {
    id?: string;
    name?: string;
    provider?: string;
    owner?: string;
...
export type AgentAPIsFilter = Camelized<APIAgentAPIsFilter>;
...
export interface SerializedPipeline {
    ...
    usage_limit: number;
    usage_count: number;
    total_usage: number;  // ✅ Add new field
    owner_id: string;     // ✅ Add new field
    organization_id: string; // ✅ Add new field
    steps: SerializedPipelineStep[];
}

export interface SerializedPipelineList {
    count: number;
    next: string | null;
    previous: string | null;
    results: SerializedPipeline[];
}

export interface SerializedPipelineStep {
    id: string;
    pipeline_id: string; // ✅ Add new field
    step_id: string;      // ✅ Changed from 'step' to 'step_id'
    order: number;
...
export interface SerializedStepRegistryParameter {
    key: string;
    param_type: string;
    default_value: any;
    description: string;
...
export interface SerializedStepRegistry {
    id: string;
    type: string;
    name: string;
    description: string;
    category: string;
    parameters: SerializedStepRegistryParameter[];
    version: string;
    created_at: string;
    updated_at: string;
}

export interface SerializedStepRegistryList {
    count: number;
    next: string | null;
    previous: string | null;
    results: SerializedStepRegistry[];
}

export interface SerializedStepExecution {
    id: string;
    step_id: string;
    execution_id: string;
    order: number;
    ...
    step_params: SerializedStepRegistryParameter[];
    log: string | null;
    started_at: string | null;
    finished_at: string | null;
}

export interface SerializedPipelineExecution {
    id: string;
    status: string;
    ...
    steps: SerializedPipelineStep[];
    step_executions: SerializedStepExecution[];
}
