/* SAMPlugin type is referenced as 'any' here to avoid circular imports with index.tsx */
import { Tensor } from 'onnxruntime-web';
import type { Job } from 'cvat-core-wrapper';
import { InitBody, WorkerAction } from './inference.worker';
import { ClickType, getModelScale, modelData, onnxToImage } from './helpers';

export async function agentsEnter(
    plugin: any,
    agentID: string | number,
    body: any,
): Promise<null | { preventMethodCall: boolean }> {
    const isSAMAgent = body?.params?.model_id === plugin.data.modelID;
    if (!isSAMAgent) return null;

    return new Promise((resolve, reject) => {
        function resolvePromise(): void {
            const frame = body?.frame_ids?.[0] || body?.frame || 0;
...
export async function agentsLeave(
    plugin: any,
    result: any,
    agentID: string | number,
    body: any,
): Promise<any> {
    const agentName = String(agentID).toLowerCase();
    const isSAMAgent = body?.params?.model_id === plugin.data.modelID;
    if (!isSAMAgent) return result;

    return new Promise((resolve, reject) => {
        const frame = body?.frame_ids?.[0] || body?.frame || 0;
        const taskID = body?.task_id || body?.taskId;
        const params = body?.params || {};
        const pos_points: number[][] = Array.isArray(params?.pos_points) ? params.pos_points : [];
        const neg_points: number[][] = Array.isArray(params?.neg_points) ? params.neg_points : [];
        const obj_bbox: number[][] = Array.isArray(params?.obj_bbox) ? params.obj_bbox : [];
