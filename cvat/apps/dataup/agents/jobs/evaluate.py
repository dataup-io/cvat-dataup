...
class AgentEvaluateQueue(BaseAgentQueue):
    ...
    def enqueue(
        ...
        *,
        ...
        rq_id = RequestId(
            action=RequestAction.EVALUATE, target=RequestTarget.TASK, target_id=task_id
        ).render()
...
class AgentEvaluateJob(BaseAgentJob):
    ...
    def __call__(cls, agent_id: str, task_id: int, **kwargs):
        ...
        # log_agent_job_start(agent_id, task_id)
        ...
        all_preds = []
        all_gt = list(chain(*ground_truth_annotations.values()))
        for frame_ids_batch in take_by(frame_ids, MAX_BATCH_SIZE):
            payload = build_infer_payload(organization_uuid, task_id, frame_ids_batch, params = params)
            try:
                ...
                batch_ground_truth_annotations = {frame_id: ground_truth_annotations.get(frame_id, []) for frame_id in frame_ids_batch}
                frame_metrics = calculate_frame_metrics(frame_ids_batch, batch_frame_predictions, batch_ground_truth_annotations)
                metric_stats["frame_metrics"].extend(frame_metrics)
        ...
        per_class_metrics = calculate_per_class_metrics(all_preds=all_preds, all_gt=all_gt, class_names=class_names)
        metric_stats["per_class_metrics"].extend(per_class_metrics)

        metric_stats["global_metrics"] = calculate_global_metrics(metric_stats["frame_metrics"], per_class_metrics)
        metric_stats.update({"agent_name": agent_id,
                "agent_version": "1.0.0",
                "task_type": "object_detection",
                "dataset_name": task_id,
                "processed_frames": processed_frames,
                "evaluation_time_sec": 35.7,})
        # log_agent_job_completion(agent_id, task_id)
        return metric_stats
