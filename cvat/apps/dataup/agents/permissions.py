...
class DataUpBenchmarkPermission(OpenPolicyAgentPermission):
    class Scopes(str, Enum):
        LIST = "list"
        VIEW = "view"
        CREATE = "create"
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.url = settings.IAM_OPA_DATA_URL + "/dataup/benchmarks/allow"
...
class DataUpLensPermission(OpenPolicyAgentPermission):
    class Scopes(str, Enum):
        VIEW = "view"
        CHAT = "chat"
        SYNC = "sync"
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.url = settings.IAM_OPA_DATA_URL + "/dataup/lens/allow"
    ...
    def _get_scopes(cls, request, view, obj) -> list[str]:
        ...
        if action == "retrieve":
            return [cls.Scopes.VIEW]
        if action == "chat":
            return [cls.Scopes.CHAT]
        if action == "sync":
            return [cls.Scopes.SYNC]
        return [cls.Scopes.VIEW]

    @classmethod
    def create(cls, request, view, obj, iam_context=None):
        if view.basename != "lens":
            return []
