...
from cvat.apps.events.export import EventsExporter, _get_analytics_data_json
...
class EventsViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        summary="Get analytics data as JSON",
        methods=["GET"],
        description="Returns analytics event data in JSON format for dashboard visualization",
        parameters=[
            *api_filter_parameters,
            OpenApiParameter(
                "format",
                location=OpenApiParameter.QUERY,
                description="Data format for analytics (raw, aggregated, or summary)",
                type=OpenApiTypes.STR,
                required=False,
            ...
            OpenApiParameter(
                "group_by",
                location=OpenApiParameter.QUERY,
                description="Group analytics data by field (scope, user_id, project_id, task_id, job_id)",
                type=OpenApiTypes.STR,
                required=False,
                enum=["scope", "user_id", "project_id", "task_id", "job_id"],
            ),
        ],
        responses={
            "200": OpenApiResponse(description="Analytics data in JSON format"),
        },
    )
