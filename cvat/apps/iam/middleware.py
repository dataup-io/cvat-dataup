...
def get_organization(request: HttpRequest):
    ...
    try:
        ...
        if org_uuid is not None and (org_slug is not None or org_header is not None):
            raise ValidationError(
                'You cannot specify "organization_uuid" query parameter with '
        ...
        if org_slug:
            ...
            organization = Organization.objects.select_related("owner").get(dataup__id=org_uuid)
    except Organization.DoesNotExist:
        raise NotFound(f"{org_slug or org_id or org_uuid} organization does not exist.")
