...
from cvat.apps.engine.frame_provider import JobFrameProvider, TaskFrameProvider
    ...
    def get(self, request, token, filename=None):
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    f"Token lookup failed (attempt {attempt + 1}/{max_retries}): {token}"
                )
        ...
        try:
            ...
            return Response(
                data=f"Error serving data: {str(e)}", status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
