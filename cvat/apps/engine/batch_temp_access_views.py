...
from cvat.apps.engine.views import _JobDataGetter, _TaskDataGetter
...
class BatchTempAccessView(APIView):
    ...
    def get(self, request, token):
        ...
        if not cache_data:
            return Response(data="Token not found or expired", status=status.HTTP_404_NOT_FOUND)

        # Check if token has expired
        current_time = int(time.time())
        if current_time > cache_data.get("expiry", 0):
            cache.delete(cache_key)
            return Response(data="Token expired", status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            if "task_id" in cache_data:
                ...
                return Response(data="Invalid token data", status=status.HTTP_404_NOT_FOUND)
    ...
    def _serve_batch_task_data(self, cache_data):
            ...
            for frame_num in frame_numbers:
                try:
                    ...
                    if hasattr(frame_response, "content"):
                        ...
                        frame_data = (
                            frame_response.getvalue()
                            if hasattr(frame_response, "getvalue")
                            else bytes(frame_response)
                        )

                    # Determine file extension based on content type
                    content_type = getattr(frame_response, "get", lambda x, default: default)(
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="task_{cache_data["task_id"]}_frames.zip"'
        )
        return response
    ...
    def _serve_batch_job_data(self, cache_data):
        ...
        try:
            ...
            return Response(data="Job not found", status=status.HTTP_404_NOT_FOUND)
        ...
        response = HttpResponse(zip_buffer.getvalue(), content_type="application/zip")
        response["Content-Disposition"] = (
            f'attachment; filename="job_{cache_data["job_id"]}_frames.zip"'
        )
        return response
